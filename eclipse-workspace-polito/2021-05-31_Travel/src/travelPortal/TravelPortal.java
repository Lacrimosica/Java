...
//R3
	public List<String> addParticipants(String code, String... names) throws TPException {
		...
		int initSize = p.getParticipants().size();
		int finalSize;
		...
		for(String s: names) {
			for(Proposal ps: proposals.values()) {
	
				if(!ps.getParticipants().isEmpty() && ps.getParticipants().contains(s)) {
					System.out.println("participant " + s + " has already done some shit");
					System.out.println("dates allready full: "+  ps.getPeriod() + " request for this date: " + p.getPeriod());
				
					if(ps.HasConflict(p.getPeriod())) {
						System.out.println("participant " + s + " has conflict");
						requests.remove(s);
						System.out.println("requests after deletion");
						System.out.println(requests);
//						
					}
				}
		...
		System.out.println("final rquests");
		System.out.println(requests);
		finalSize = initSize + requests.size();
		System.out.println("size allowance: " + p.getMaxNP() + "-" + p.getMinNP() + " ? final size = " + finalSize);
		if(finalSize > p.getMaxNP() || finalSize < p.getMinNP()) {
			throw new TPException("number of participants doesn't fit the proposal");
		}
		
		p.addParticipants(requests);
		try {
			if(requests.size() != names.length) {
				throw new TPException("date conflict");
			}
		}
		
		catch(TPException tp) {};
	...
	public int getIncome(String code) {
		...
		return p.getParticipants().size() * p.getTotalPrice();
	}
...
//R4
	public String addRatings(String code, int... evaluations) throws TPException {
		...
		if(p.getParticipants().size() != evaluations.length) {
			throw new TPException();
		}
		
		for(int i=0; i< evaluations.length; i++) {
			//adding to database part
			p.addRating(p.getParticipants().get(i), evaluations[i]);
			
			//printing part
			bf.append(p.getParticipants().get(i)).append(":").append(evaluations[i]);
	...
	public SortedMap<String, Integer> getTotalRatingsForParticipants() {
		...
		return proposals.values().stream()
			.flatMap(p ->p.getRatings().entrySet()
					.stream()
						.map(r -> new HashMap<String, Integer>(r.getKey, r.getValue))
						.collect(groupingBy(Map.Entry<String, Integer>::getKey, toList() )));
								
	}

//R5
	public SortedMap<String, Integer> incomeForActivityTypes() {
		travelAgencies.values().stream().map(ta -> ta.getActivities().)
