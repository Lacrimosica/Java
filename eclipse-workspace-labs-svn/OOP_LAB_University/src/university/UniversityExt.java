...
public class UniversityExt extends University {
	...
	public void exam(int studentId, int courseID, int grade) {
		...
		Course c = courses.get(courseID - CREG);
		
		s.giveExam(c, grade);
		c.registerGrade(s, grade);
	...
	public String studentAvg(int studentId) {
		Student s = students.get(studentId- SREG);
		if(s.average() != -1) {	
			return studentId + " : " + s.getAverage();
		}
		return  s.getMatricola() + " hasn't taken any exams";
	...
	public String topThreeStudents() {
		...
		Map<Double, Student> deansList= new TreeMap<Double, Student>(Collections.reverseOrder());
		StringBuffer result = new StringBuffer();
		for(Student s: students) {
			s.average();
			deansList.put(s.getAverage(), s);
		}
		
		System.out.println();
		
		for(Student s:deansList.values()) {
			
			result.append(s.getName());
			result.append(" :");
			result.append(s.getAverage());
			result.append('\n');
