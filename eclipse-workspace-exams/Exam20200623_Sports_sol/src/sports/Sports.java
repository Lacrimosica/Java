...
//    	return activities.entrySet().stream().map(e ->e.getKey()).collect(toList());
    	return activities.keySet().stream().collect(toList());
...
//        
    	...
    	userRatings.values().stream().flatMap(l -> l.stream())
    			.map( r -> r.getStars())
    			.collect(averagingInt(Integer::sum));
