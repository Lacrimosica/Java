...
public class Delivery {
	...
	private SortedSet<String> restaurantCategories;
	public Delivery() {
		restaurantCategories = new TreeSet<>();
	}

	
	public Restaurant getRestaurant(String name) {
		return Directory.getInstance().getRestaurant(name);
	}
	// R1
    /**
	...
	public List<String> getCategories() {
		return new ArrayList<String> (restaurantCategories);
	}
	...
	public void addRestaurant (String name, String category) throws DeliveryException {
		...
		if(!restaurantCategories.contains(category)) {
			throw new DeliveryException("category not defined");
		}
		
		Directory.getInstance().addRestaurant(new Restaurant(name, category));
	}
	...
	public List<String> getRestaurantsForCategory(String category) {
        return Directory.getInstance().getRestaurants()
        		.stream().filter(r -> r.getCategory().equals(category))
        			.map(r -> r.getName())
	...
	public void addDish(String name, String restaurantName, float price) throws DeliveryException {
		Directory.getInstance().getRestaurant(restaurantName).addDish(name, price);
	}
...
//        									Collectors.toList()));
		...
		Directory.getInstance().getRestaurants()
				.stream()
					.flatMap(r -> r.getDishes().stream()).forEach(System.out::println);
	...
	public List<String> getDishesForRestaurant(String restaurantName) {
		return Directory.getInstance().getRestaurant(restaurantName).getDishNames();
	}
        ...
        return  Directory.getInstance().getRestaurants()
        		.stream()
        			.filter(r -> r.getCategory().equals(category))
        					.flatMap(r -> r.getDishNames().stream()).collect(Collectors.toList());
	}
	...
	public int addOrder(String dishNames[], int quantities[], String customerName, String restaurantName, int deliveryTime, int deliveryDistance) {
	    return Directory.getInstance().getRestaurant(restaurantName)
	    .addOrder(dishNames, quantities, customerName, restaurantName, deliveryTime, deliveryDistance);
	...
	public List<Integer> scheduleDelivery(int deliveryTime, int maxDistance, int maxOrders) {
        Directory.getInstance().getRestaurants().stream().map(r -> r.getOrders().stream())
	}
